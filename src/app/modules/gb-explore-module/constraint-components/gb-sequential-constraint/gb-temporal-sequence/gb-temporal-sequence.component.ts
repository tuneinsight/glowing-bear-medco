...
export class GbTemporalSequenceComponent implements OnInit, OnDestroy {
  ...
  _when: SelectItem[] = [
    { label: 'before', value: ApiI2b2When.less },
    { label: 'before or same time as', value: ApiI2b2When.lessequal },
    { label: 'same time as', value: ApiI2b2When.equal }
  ]
  ...
  constructor(private config: DynamicDialogConfig, private ref: DynamicDialogRef) {
    ...
    if (!!(config)) {
      ...
      this.whenOperator = sequenceInfo.when
  ...
  set whenOperator(whenOperator: ApiI2b2When) {
    this._whenOperator = whenOperator

  }

  get whenOperator(): ApiI2b2When {
    return this._whenOperator
  }
